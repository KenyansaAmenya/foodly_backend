...
module.exports = {
    addRestaurant: async(req, res) => {
        ...
        try {
            ...
            res.status(201).json({staus: true, message: "Restaurant successfully created"})
        } catch (error) {
            res.status(500).json({status: false, message: "Error creating restaurant", error: error.message})
        }
    },

    serviceAvaillability: async(req, res) => {
        ...
        try {
            ...
            restaurant.isAvaillable = !restaurant.isAvaillable

            await restaurant.save()
            res.status(200).json({status: true, message: "Availlability successfully toggled", isAvaillable: restaurant.isAvaillable})
        } catch (error) {
    ...
    getRestaurant: async (req, res) => {
     ...
     // pipeline for getting random restaurants 
